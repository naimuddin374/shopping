...
exports.list = async (req, res) => {
  try {
    ...
    const data = await Review.find(query)
      ...
      .skip((page - 1) * limit)
      .limit(limit);
    const totalDocument = await Review.countDocuments(query);
...
exports.getById = async (req, res) => {
  try {
    let result = await Review.findById(req.params.id).populate([
      "product",
      "user",
    ]);
...
exports.insert = async (req, res) => {
  ...
  try {
    ...
    await schema.save();
    const result = await Review.findById(schema._id).populate([
      "product",
      "user",
    ]);
